...
import edu.stanford.kellenvu.mymaps.databinding.ActivityDisplayMapBinding
...
class DisplayMapActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        // Build initial bounds of map
        val boundsBuilder = LatLngBounds.Builder()
        // Iterate through each place
        for(place in userMap.places) {
            val latLng = LatLng(place.latitude, place.longitude)
            boundsBuilder.include(latLng) // Consider this when building bounds
            mMap.addMarker(MarkerOptions().position(latLng).title(place.title).snippet(place.description))
        }
        // Apply initial bounds
        mMap.animateCamera(CameraUpdateFactory.newLatLngBounds(boundsBuilder.build(), 1000, 1000, 200))
        // 1000, 1000 specifies how much to zoom in
